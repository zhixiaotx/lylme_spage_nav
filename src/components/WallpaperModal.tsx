  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-hidden">
      ...
      >
        ...
        <div className={`px-6 py-4 flex items-center justify-between border-b ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold tracking-tight flex items-center gap-2">
                <span>壁纸中心与背景图库</span>
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-sky-500/20 text-sky-400 font-medium border border-sky-500/30">
                  4K / Bing / 渐变 / 云端同步
                </span>
              </h2>
              <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                支持微软 Bing 每日大片、精选摄影画廊、多色渐变及参数实时调节
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={isRolling}
              className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-sky-600 hover:bg-sky-500 text-white flex items-center gap-1.5 shadow-md shadow-sky-600/20 active:scale-95 transition-all disabled:opacity-50"
              title="从精选图库随机挑选一张"
            >
              <Dices size={14} className={isRolling ? 'animate-spin' : ''} />
              <span>{isRolling ? '挑选壁纸中...' : '随机换一张'}</span>
            </button>
        ...
        {/* Categories Bar */}
        <div className={`px-6 py-2.5 flex items-center gap-2 overflow-x-auto no-scrollbar border-b ${
          isDarkMode ? 'border-white/10 bg-slate-900/30' : 'border-slate-200/80 bg-slate-100/50'
        }`}>
          <button
            type="button"
